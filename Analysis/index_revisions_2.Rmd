...
We can then plot this data to show how our ability to predict which target will be selected next varies throughout a trial. We can see a difference between *feature* and *conjunction* foraging, with *feature* foraging being more predictable (Figure 1). 
...
#       plt_b2 / (plt1 + plt2) + plot_annotation(tag_levels = list(c('(a)', '(b)', '(c)'))) +
...
ggplot(a_acc, aes(x= condition, y = accuracy, fill = condition)) + 
  ...
  theme(legend.position="none")-> plt_a
...
plt_b <- ggplot(fit_plt, aes(value, accuracy, colour = condition)) + 
...
#  scale_shape_manual(values = c(19, 4), guide = "none") + 
#  xlab("bP parameter value") +
#  theme(legend.position = "none") + 
#   scale_y_continuous("model accuracy",  
#                     limits = c(0.35, 0.8), breaks = seq(0.3, 0.8, 0.1))

#ggsave("../Figures/qjep_indiv_diff.pdf", 
#       plt_a + plt_b + plot_annotation(tag_levels = list(c('(a)', '(b)'))) +
#         plot_layout(widths = c(1, 2)), width = 12, height = 4)
```
...
After removing them, we can see that individual differences in proximity tuning account for nearly all of the differences in predictability from person to person, and also between the *feature* and *conjunction* conditions (see Figure 4).
...
```{r, message = FALSE, warning = FALSE, fig.cap="The relationship between the time taken to complete a trial and how predictable it is. Dots show empirical data, and the shaded ribbon shows the fixed-effects of a multi-level model. The lines give posterior samples from each individual observer."}
...
```
This relationship appears to hold true if you look at how long it takes participants to select individual targets as well (Figure 5).


```{r, message = FALSE, warning = FALSE, fig.cap = "Figure 5: The relationship between the time taken to complete a single target selection and how predictable it is."}
...
Let's score every trial by how well the model captures behaviour (Figure 6), and then plot some different examples.
...
```{r fig.height=3.5, fig.cap = "Figure 6: Histogram of how predictiable individual trials are, defined as how often the model assigned the largest weight to the target that was then selected."}
...
Let's plot some examples of highly predictable (top 5%), typical (middle 10%) and unpredictable (bottom 5%) trials.
...
We can see that for these highly predictable trials (Figure 7), we make very few mistakes, and those mistakes that we do make appear to be related to some sort of shortest-path optimization on the part of the participant. 

```{r fig.cap = "Figure 7: A random selection of *good* trials. When participants diverge from the model's prediction, it appears to be due to some form of path-length optimisation."}
...
Here are some typical trials (Figure 8). Possible we have the path-length minimization issue again. 

```{r fig.cap = "Figure 8: A random selection of *typical* trials. Along with path-minimising behaviour, we can also see some disagreement around switching item type."}
...
Now let's look at less predictable (bottom 5%-tile) trials (Figure 9). Even here, the model does a relatively good job. 

```{r fig.cap = "Figure 9: A random selection of *hard* trials. It is harder to make out much of a pattern here, but we can see that the human participants jump around more."}
...
#      ( plts1[[1]] + plts1[[2]]) /( plts3[[1]] + plts3[[3]])  + plot_annotation(tag_levels = list(c('(a)', '(b)', '(c)', '(d)'))),
#       width = 8, height = 8)
...
```{r, fig.height=3.5, fig.cap = "Figure 10: (*left*) Histogram comparing the empirical path lengths to simulations from our fitted model. Difference indicates the human path length - the simulated path length. (*right*) Histogram comparing the median path lengths per participant.", message=FALSE}
...
We can see that overall, the model is not making substantially longer paths through the items than our human participants (Figure 10). However, there is some variability in that some participants routinely select paths shorter than our model would, and others do not.

Perhaps this is related to predictability (i.e., how well the model can predict a participant's trial level behaviour - Figure 11)?

```{r fig.height=3, fig.cap = "Figure 11: (*left*) A histogram showing the proportion of trials on which each participant managed a shorter path through the targets than the model's prediction. (*right*) Scatter plot showing the relationship between predictability (accuracy) and whether the model over or under estimates path length. Xs mark the outlier participants discussed above."}
...
```{r}
#ggsave("../Figures/path_length.pdf", plt2 + plt2b + plot_annotation(tag_levels = list(c('(a)', '(b)'))), width = 8, height = 3)
```
...
Previous work (Clarke and Tatler, 2014) has demonstrated that the initial fixation in scene viewing can be well described with truncated Gaussian distribution. Here we explore using a similar idea to account for starting locations (Figure 12). 
...
```{r fig.height = 3.5, fig.cap="Figure 12: (*left*) Density plot for initial target selections, over all participants and all trials. (*right*) Median $x$ and $y$ coordinates for the initial target selection for each participant."}
...
Can we model this as multi-level beta distribution? Yes, although it seems like people are in one of two clusters (Figure 13). 

```{r fig.cap = "Figure 13: The mean posterior plot for the truncated normal distriubtions for (*left*) $x$ and (*right*) $y$. We can see that a two component mixture model will capture most of the variance in intial target selection.", warning = FALSE}
...
create_post_plot("x") + create_post_plot("y") + plt_mu + plot_annotation(tag_levels = list(c('(a)', '(b)', '(c)'))) #& theme_bw() 
...
It looks like we can simplify the model somewhat by using a two component mixture model instead (Figure 14).

```{r, fig.height = 6, fig.cap = "Figure 14: (*top*) Beta distributions for $x$ and $y$ coordinated"}
...
(plt_x + plt_y) + plt_lambdas + plot_annotation(tag_levels = list(c('(a)', '(b)', '(c)')))
...
plt1 / (plt2 + plt3)  + plot_layout(guides = "collect")  + plot_annotation(tag_levels = list(c('(a)', '(b)', '(c)')))
