...
a %>% select(observer, condition, trial, found, selected_max) %>%
  ...
  summarise(time_taken = max(RT),
            ...
            .groups = "drop") %>%
  filter(time_taken < 60)  ->timedat

%>%
  ggplot(aes(time_taken, acc, colour = condition)) + 
  geom_jitter(alpha = 0.2, height = 0.005, width = 0) +
  geom_smooth(method = "lm") + 
...
a %>% select(observer, condition, trial, found, selected_max) %>%
  ...
  mutate(
    time_bin = cut(RTd,breaks = seq(0, 2, 0.25), labels = FALSE)) %>%
