...
ggsave("../Figures/prpic_model_params.pdf",  width = 8, height = 6)
```
...
a %>% group_by(condition, observer, found) %>%
  summarise(meanb = mean(b),
            prop_best = mean(selected_max), .groups = "drop") -> a_agg
```
...
a %>% mutate(b_bin = cut(max_b, breaks = my_breaks, labels = FALSE)) %>%
  ...
  mutate(b_bin = my_breaks[b_bin]) -> a_cut
```
