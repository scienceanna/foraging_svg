...
## Are more predictable people faster at the task?

```{r}
drt <- filter(d, found == 40) %>% group_by(observer, condition) %>%
  summarise(median_rt = median(RT), .groups = "drop") %>% 
  full_join(dm, by = c("observer", "condition"))

ggplot(drt, aes(accuracy, median_rt, colour = condition)) +
         geom_point(alpha = 0.5) + 
  geom_smooth(method = "lm") 

summary(lm(median_rt ~ accuracy*condition, data = drt))
...
create_post_plot <- function(dm) {
  ...
  pmap(filter(am,  dim == dm) %>% select(mean, sd), dtruncnorm, a = 0, b= 1, x = z) %>%
    ...
    ggplot(aes(x = z, y = v, group = observer)) + 
...
am %>% select(-sd) %>% 
  ...
  ggplot(aes(x, y)) + 
  geom_point(alpha = 0.5) + 
  geom_line(data = tibble(), aes(x = c(0, 1), y = c(0, 0)), color = "grey") +
  geom_line(data = tibble(), aes(x = c(0, 1), y = c(1, 1)), color = "grey") +
  geom_line(data = tibble(), aes(x = c(0, 0), y = c(0, 1)), color = "grey") +
  geom_line(data = tibble(), aes(x = c(1, 1), y = c(0, 1)), color = "grey") -> plt_m

create_post_plot("x") + create_post_plot("y") + plt_m
ggsave("../Figures/init_sel_mdl.pdf", width = 10, height = 3)
```

- Do we want to simplify this model to something in which people have some mix of a (fixed) centre and corner bias?

Do participants who differ in terms of their initial target selection differ in other ways too?

```{r echo = TRUE, fig.cap = "Whether participants have a central or corner bias for the intiial target selection appears to be largely independent of their other model parameters."}
full_join(fit, filter(am, dim == "x"), by = "observer") %>%
  pivot_longer(c(pA, pS, bP, bM, accuracy), 
               names_to = "statistic", values_to = "value") %>%
  mutate(intial_sel = if_else(mean < 0, "corner", "central")) %>%
  ggplot(aes(statistic, value, fill = intial_sel)) +
  geom_boxplot(alpha = 0.5) + 
  facet_wrap(~statistic, scales = "free", nrow = 1) +
  theme(legend.position = "bottom") + 
   scale_fill_manual("intial target selection",
                     values = c(col3[2], col3[3]))
```
