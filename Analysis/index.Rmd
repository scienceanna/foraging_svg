...
```
How well does the model do in guessing the first target in a trial?
...
plt_dat %>% filter(dim == "x") %>%
  ...
  geom_path(alpha = 0.1) + 
  facet_wrap(~dim) + 
  theme(legend.position = "none") -> plt_x
...
plt_dat %>% filter(dim == "y") %>%
  ...
  facet_wrap(~dim) + 
  theme(legend.position = "none") +
  scale_x_continuous("y") -> plt_y


gather_draws(m, lambda[observer]) %>%
  ggplot(aes(.value, group = observer)) + 
  geom_density(alpha = 0.25, fill = "grey") -> plt_lambdas
...
weights2 %>% group_by(observer) %>%
  summarise(accuracy = mean(max_item == 1)) %>%
  full_join(lambdas) %>%
  rename(lambda = "value") %>%
  ggplot(aes(lambda, accuracy)) + 
  geom_point() +
  geom_smooth(method = "lm", colour = "black") -> plt3
